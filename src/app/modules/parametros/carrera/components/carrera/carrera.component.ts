...
export class CarreraComponent implements OnInit, OnDestroy {
  ...
  // permisos locales
  eliminarPermiso = 'eliminar_carrera';
  consultarPermiso = 'consultar_carrera';
  editarPermiso = 'editar_carrera';
  reportePermiso = 'reporte_carrera';
  ...
  cargarTabla(size: number, current: number): void {
    this.carrera$ =  this.servicio.getPaginated(size, current).subscribe((res: any) =>
      {
  ...
  cargarTablaFiltrada(size: number, current: number, nombre: string): void {
    this.carrera$ =  this.servicio.getFiltered(size, current, nombre).subscribe((res: any) =>
    {
     this.dataSource = res.data; console.log(res);
     this.length = res.total;
    });
  ...
  ver(carrera: Carrera): void {
    if (this.verificarPermisos(this.consultarPermiso)) {
      this.dialog.open(ModalComponent, {width: '40vw', data:  carrera });
    }
  }
  ...
  eliminar(data: any): void {
    if (this.verificarPermisos(this.eliminarPermiso)) {
      ...
      if (!estado) {
        ...
        Swal.fire(
          'Error',
          `Debes deshabilitar la carrera antes de eliminarla`,
          'error'
        );
      }
    }
