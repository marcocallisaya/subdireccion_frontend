...
export class UbicacionComponent implements OnInit, OnDestroy {
  ...
  // permisos locales
  eliminarPermiso = 'eliminar_ubicacion';
  consultarPermiso = 'consultar_ubicacion';
  editarPermiso = 'editar_ubicacion';
  reportePermiso = 'reporte_ubicacion';
  ...
  displayedColumns: string[] = ['codigo', 'disponibilidad', 'color' , 'estado'];
  ...
  cargarTabla(size: number, current: number): void {
    this.ubicacion$ =  this.servicio.getPaginated(size, current).subscribe((res: any) =>
      {
  ...
  cargarTablaFiltrada(size: number, current: number, nombre: string): void {
    this.ubicacion$ =  this.servicio.getFiltered(size, current, nombre).subscribe((res: any) =>
    {
     this.dataSource = res.data; console.log(res);
     this.length = res.total;
    });
  ...
  eliminar(data: any): void {
    if (this.verificarPermisos(this.eliminarPermiso)) {
      ...
      if (!estado) {
        ...
        Swal.fire(
          'Error',
          `Debes deshabilitar la ubicacion antes de eliminarla`,
          'error'
        );
      }
    }
