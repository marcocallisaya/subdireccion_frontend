...
export class TipoTramiteComponent implements OnInit, OnDestroy {
  ...
  // permisos locales
  eliminarPermiso = 'eliminar_tipo_tramite';
  consultarPermiso = 'consultar_tipo_tramite';
  editarPermiso = 'editar_tipo_tramite';
  reportePermiso = 'reporte_tipo_tramite';
  ...
  ver(tipo: TipoTramiteService): void {
  ...
  eliminar(data: any): void {
    if (this.verificarPermisos(this.eliminarPermiso)) {
      ...
      if (!estado) {
        ...
        Swal.fire(
          'Error',
          `Debes deshabilitar el tipo de tramite antes de eliminarlo`,
          'error'
        );
      }
    }
